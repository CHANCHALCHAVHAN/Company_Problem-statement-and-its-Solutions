...
public class Main {
    public static int user_logic(int n, int[] tasks, int W) {
        int low = Arrays.stream(tasks).min().getAsInt();
        int high = Arrays.stream(tasks).sum();
        int ans = 0;
    ...
    private static boolean canAllocate(int[] tasks, int W, int minWorkload) {
        int workersUsed = 0;
        int currentSum = 0;

        for (int task : tasks) {
            currentSum += task;
            if (currentSum >= minWorkload) {
                workersUsed++;
                currentSum = 0;
            }
        }
